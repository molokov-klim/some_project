...
# TODO
# снять номер последнего ФД
# отчет о регистрации +ДТО
# добить электронные чеки +ДТО
# отчете об изменении параметров регистрации +ДТО
# отчете об открытии смены
# отчете о текущем состоянии расчетов
# кассовом чеке (БСО) + приход расход + нал безнал комбо
# кассовом чеке коррекции (БСО коррекции) + приход расход + нал безнал комбо + сам или по указанию
# отчете о закрытии смены
# отчете о закрытии фискального накопителя +ДТО
@print_test_info("Проверка отправки ОФД всех типов ФД")
@allure.title("Проверка отправки ОФД всех типов ФД")
@allure.feature("Сигма Касса")
@allure.story("")
@allure.id("")
def test_RNDAGB_T93_check_receipts_sent_to_OFD(sk_without_install_fixture):
    # Инициализация
    app = sk_without_install_fixture

    # Открытие смены
    app.input_pin_code(number=config.SIGMA_KASSA_PIN_CODE, timeout=3)
    app.open_shift(cash='0', timeout=30)
    app.clear_cart()
    ...
    # Закрытие смены
    app.close_shift_with_zero_FD(timeout=600)
